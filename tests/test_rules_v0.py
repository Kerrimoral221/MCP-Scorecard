...
from mcp_trust.models import FindingCategory, FindingLevel, NormalizedServer, NormalizedTool
...
def test_duplicate_and_missing_description_rules_emit_expected_findings() -> None:
    server = _server(
        NormalizedTool(name="echo"),
        NormalizedTool(name="echo", description="Echo back text."),
    )
    ...
    assert duplicate_finding[0].score_impact == 20
    assert duplicate_finding[0].tool_name == "echo"
    ...
    assert missing_description_finding[0].tool_name == "echo"
    assert "description=<missing>" in missing_description_finding[0].evidence


def test_vague_and_weak_schema_rules_emit_expected_findings() -> None:
    server = _server(
        NormalizedTool(
            name="do_it",
    ...
    assert weak_schema_findings[0].severity is FindingLevel.WARNING
    assert weak_schema_findings[0].category is FindingCategory.INPUT_SCHEMA
    assert weak_schema_findings[0].score_impact == 10
    assert "additionalProperties=True" in weak_schema_findings[0].evidence
...
def test_capability_rules_emit_expected_findings() -> None:
    ...
    assert [finding.tool_name for finding in exec_findings] == ["exec_command"]
    assert exec_findings[0].severity is FindingLevel.ERROR
    assert exec_findings[0].category is FindingCategory.CAPABILITY
    assert exec_findings[0].score_impact == 20
    assert "input_keys=['command']" in exec_findings[0].evidence

    assert [finding.tool_name for finding in fs_findings] == ["write_file"]
    assert fs_findings[0].severity is FindingLevel.ERROR
    assert fs_findings[0].category is FindingCategory.CAPABILITY
    assert fs_findings[0].score_impact == 20
...
def test_v0_ruleset_finds_expected_issues_on_insecure_server() -> None:
    ...
    assert report.score.penalty_points == 60
    assert report.score.final_score == 40
    assert report.score.category_breakdown[report.findings[0].score_category].score == 40
